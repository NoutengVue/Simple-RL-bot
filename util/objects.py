...
class GoslingAgent(BaseAgent):
    ...
    def get_closest_large_boost(self):
        available_boosts = [boost for boost in self.boost if boost.lage and boost.active]
        closest_boost = None
    ...
    def is_in_front_of_ball(self):
        b1 = abs(self.ball.location.y - self.foe_goal.location.y)
        b2 = abs(self.me.location.y - self.foe_goal.location.y)
        if b2 < b1 + 1000:
            return True
        return False
